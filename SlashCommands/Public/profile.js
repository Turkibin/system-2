...
module.exports = {
    ...
    options: [
        {
            name: 'user',
            description: 'The user to display profile for',
            type: 'USER',
            required: false
        }
    ],
    ...
    run: async (client, interaction, args) => {
        ...
        try {
            ...
            if (!backgroundImageUrl) {
                backgroundImageUrl = Data.get(`${interaction.guild.id}_background`);
            }
            ...
            const inviteCount = await getInviteCount(interaction.guild, member.id);
            ...
            try {
                ...
                return interaction.followUp("Error loading background image. Please make sure the URL is valid.");
            }
            ...
            await drawServerInfo(ctx, interaction.guild);
            ...
            interaction.followUp("❌ An error occurred while generating the profile card.");
        }
    }
};
...
async function getTotalRank(userId) {
    ...
    const sortedEntries = Object.entries(totalPointsEntries)
        .sort(([, valueA], [, valueB]) => valueB - valueA);

    const userRank = sortedEntries.findIndex(([key]) => key === userId) + 1;
...
async function drawAvatarAndStatus(ctx, member, status) {
    ...
    if (status === 'idle') {
        ctx.beginPath();
        ctx.fillStyle = '#2f3136';
        ctx.arc(statusX + 1, statusY - 1, 6, 0, Math.PI * 2);
        ctx.fill();
    }
}

function drawUsername(ctx, member) {
    const displayName = member.displayName; // Changed from member.user.displayName
...
function drawProgressBar(ctx, textPoints, voicePoints, totalPoints) {
    ...
    roundRect(ctx, progressBarX, progressBarY, config.progressBarWidth, config.progressBarHeight, config.progressBarRadius, true);
    ...
    const combinedGradient = ctx.createLinearGradient(progressBarX, progressBarY, progressBarX + totalWidth, progressBarY);
    ...
    roundRect(ctx, progressBarX, progressBarY, totalWidth, config.progressBarHeight, config.progressBarRadius, true);
    ...
    if (voiceWidth > 70) {
        ctx.fillText(`Voice: ${voicePoints}`, progressBarX + voiceWidth / 2, progressBarY + 15);
    }
    if (textWidth > 70) {
        ctx.fillText(`Text: ${textPoints}`, progressBarX + voiceWidth + textWidth / 2, progressBarY + 15);
    }
...
async function drawServerInfo(ctx, guild) {
    ...
    const serverAvatarURL = guild.iconURL({ format: "png", size: 1024 });
